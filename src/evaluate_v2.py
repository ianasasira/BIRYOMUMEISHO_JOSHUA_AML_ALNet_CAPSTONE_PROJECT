...
def load_model():
    ...
    if not model_path.exists():
        raise FileNotFoundError("No trained model found. Run train_v2_alnet.py first.")
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    plt.title("ALNet v2 — Confusion Matrix (Test Set)")
...
def plot_roc_curve(y_true, y_score, save_path):
    ...
    plt.figure(figsize=(6, 5))
    plt.plot(fpr, tpr, label=f"ALNet v2 (AUC = {auc:.4f})", linewidth=2)
    plt.plot([0, 1], [0, 1], "k--", linewidth=1, label="Random (AUC = 0.5)")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate (Recall)")
    plt.title("ROC Curve — ALNet v2 (Test Set)")
...
def evaluate():
    ...
    _, _, test_loader = create_dataloaders(batch_size=BATCH_SIZE, num_workers=0)
    ...
    recall_val = recall_score(labels, preds, average="binary", zero_division=0)
    ...
    results = {
        ...
        "sensitivity_recall": round(float(recall_val), 4),
    ...
    print(f"\n{'='*50}")
    print("ALNet v2 Evaluation — Test Set")
    print(f"{'='*50}")
    print(f"Test samples: {results['test_size']} "
          f"(Myeloblast={results['test_positives']}, Monocyte={results['test_negatives']})")
    print(f"\nConfusion Matrix:")
    ...
    print(f"  Sensitivity/Recall:  {recall_val:.4f}")
